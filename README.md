...
### 📝 模式 A：语料生成 / 重写 (`generate`)

把你那些简陋的、敷衍的、一句话糊弄人的答案，重写成详细的、分步的、让人看了就想点赞的高质量回答。不合格的自动打回重写，最多循环 N 轮直到满意为止。

### 📊 模式 B：语料质量评估 (`evaluate`)

不改数据，只打分。从「准确性」「完备性」「逻辑严谨性」「格式依从性」四个维度逐条评审，给出分数和改进建议。适合数据标注和质检场景。

### 🧠 模式 C：CoT 思维链标注 (`cot`)

**专为 LLM 推理能力训练设计。** 给定数学/物理/逻辑题目，自动生成带步骤标签的 Chain-of-Thought 推理链数据。每个步骤都有类型标签（审题/建模/公式推导/代入计算/验证/结论），生成后由 CoT-Evaluator **逐步骤验证**正确性——不是整体打分，而是检查每一步的计算、逻辑和公式是否正确。错误步骤会被精确定位并打回重写。

```bash
python main.py cot -i math_problems.jsonl -o cot_data.jsonl "生成严格的数学分步推导，每步标注步骤类型"
```
...
| **CoT 逐步骤验证** | 不只看最终答案，对推理链每一步独立检查计算和逻辑 |
