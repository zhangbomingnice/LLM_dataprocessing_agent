...
写完初版之后，又加了几个我觉得挺重要的模块：

### CoT 思维链标注

专门给数学/物理题生成分步推导数据（训练 LLM 推理能力的核心燃料）。每步标注类型（审题/建模/公式推导/代入计算/验证/结论），Evaluator 逐步骤验证——不是看总分，而是检查每一步有没有算错、有没有逻辑跳跃。

### Self-Consistency 多路采样

同一道题生成 5 条不同推理路径，最终答案投票选最可信的。这个方法来自 Wang et al., 2022 的论文，是目前提升 CoT 可靠性最有效的手段之一。

### SymPy 数学验算

数学计算不能光靠 LLM "感觉"对不对。把关键表达式丢给 SymPy 做符号计算，这是确定性验证，不存在概率误差。

### 数据增强 & 去重 & 难度分级

- `--augment`：从一道题自动生成多个变体（数值替换、条件变换），500 题可以扩充到 2000 题
- `--dedup`：精确去重 + 近似去重，消灭重复数据
- `--difficulty`：自动标注 easy/medium/hard
